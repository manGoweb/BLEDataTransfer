...
bool BDTMessageService::isStartingPacket(uint8_t *buffer) {
...
bool BDTMessageService::processFirstPacket(uint8_t *buffer, uint16_t size) {
...
bool BDTMessageService::processSecondPacket(uint8_t *buffer, uint16_t size) {
...
void BDTMessageService::addToMessage(uint8_t *buffer, uint16_t size) {
...
BDTMessageService::BDTMessageService(){
    Serial.println("YOP");
    this->recievedMsgLen = 0;
    this->actualMsgLen = 0;
...
bool BDTMessageService::isMessageComplete() {
    return this->messageComplete;
}

bool BDTMessageService::BDTMessageService::isMessageValid() {
    return this->messageValid;
}

char* BDTMessageService::getMessageBuffer() {
    return this->messageBuffer;
}

bool BDTMessageService::areEqual(char *hash1, char *hash2, uint8_t size) {
    for(uint8_t i = 0; i < size; i++) {
        if(hash1[i] != hash2[i]) {
            return false;
        }
...
bool BDTMessageService::processMessage(uint8_t *buffer, uint16_t size) {
...
void BDTMessageService::sendStringOnParts(String string, uint16_t msg_handler) {
